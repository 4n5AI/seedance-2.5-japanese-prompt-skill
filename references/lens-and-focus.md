...
35mm判フルサイズ（36×24mm、対角 43.27mm）換算。**直線を直線に写すレクチリニア設計を前提とした計算値**で、計算式 `画角 = 2 × arctan(センサー寸法 ÷ (2 × 焦点距離))` から算出し、50mm 行が公開されている実測表と一致することを確認している。
...
- **8mm級の実在レンズは魚眼設計（対角180°級）が主流**で、その場合は上の計算式が成立しない。12mm級にはレクチリニア設計（対角約122°）も一般的なので、上表の延長として扱える。魚眼を狙うときは画角の数値ではなく `fisheye lens, extreme barrel distortion, circular frame` のように見え方で書く。
...
| **広角 24–40mm** | 環境の中に人物を置く。臨場感。ドキュメンタリー、歩き回る人物の追従 | `wide shot`, `35mm-lens look`, `environment held in frame`, `handheld`, `spatial depth` |
| **標準 40–58mm** | 誇張も圧縮もない自然な距離感。会話・対話 | `normal lens`, `natural perspective`, `neutral depth`, `conversational framing` |
| **中望遠 58–135mm** | 背景を引き寄せて被写体を浮かせる。顔のバランスが整う。ポートレートの本命 | `short telephoto`, `85mm portrait-lens look`, `shallow depth of field`, `subject isolated from a softly compressed background`, `creamy bokeh` |
| **望遠 135mm以上** | 奥行きの手がかりが消え、空間が強く平坦化する。離れた要素が密着して見える。覗き見・監視・孤立。望遠鏡の見え方を模すため、監視や盗み見を扱う映画の定番 | `long telephoto`, `heavy telephoto compression`, `flattened space`, `voyeuristic`, `observed from a distance` |
...
深度が**浅くなる**条件：絞りを開ける（F値が小さい）／焦点距離が長い／被写体に近い。同じ画角・同じF値で比べればセンサーが大きいほど浅くなる（光学的に確立した関係）が、日本語の主要な解説はF値・焦点距離・撮影距離の3要素で説明することが多い。
...
- 本ファイルの光学の記述（画角の計算式、区分、シネマレンズの仕様、アナモルフィックのスクイーズ比、ヴィンテージレンズの描写）は、公開されている解説と物理の計算式に基づく一般的な撮影知識であり、**Seedance の公式資料には含まれない**。
- 画角表とクロップファクターは上の計算式から算出した値で、50mm 行を公開されている実測表と突き合わせている。それ以外の行を個別に実測表と照合してはいないため、四捨五入で±0.1°程度ずれることがある。
- 焦点距離帯の境界（超広角／広角／標準／中望遠／望遠）は流派によってずれる。§1の区分列と§2の帯は同じ範囲に揃えているが、厳密な規格ではない。
